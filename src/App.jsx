...
function getMotivations(income,categories,savings,fixedExpenses,balanceCarryover){
  ...
  fixedExpenses.forEach(fx=>{
    ...
    if(days!==null&&days<=3)msgs.push({type:"warn",icon:"📅",text:`${fx.emoji} ${fx.name} ($${fmt(fx.amount)}) is due in ${days===0?"today":days===1?"1 day":`${days} days`}!`});
  });
...
export default function App(){
  ...
  const [spendAmt,setSpendAmt]=useState(""); const [spendNote,setSpendNote]=useState(""); const [spendCat,setSpendCat]=useState(null);
  ...
  const [depAmt,setDepAmt]=useState(""); const [editDepIdx,setEditDepIdx]=useState(null); const [editDepAmt,setEditDepAmt]=useState("");
  const [fixName,setFixName]=useState(""); const [fixEmoji,setFixEmoji]=useState("🏠"); const [fixAmt,setFixAmt]=useState(""); const [fixDue,setFixDue]=useState(""); const [showFixEmoji,setShowFixEmoji]=useState(false); const [editFixIdx,setEditFixIdx]=useState(null); const [editFixAmt,setEditFixAmt]=useState(""); const [editFixDue,setEditFixDue]=useState("");
  ...
  function logSpend(catName,amt,note){
    ...
    persist(updated,null,null,null);showToast(`$${fmt(amt)} logged ✓`);
  }
    ...
    <div style={{minHeight:"100vh",background:"#07090f",color:"#e2e4f0",fontFamily:"'DM Sans',sans-serif",paddingBottom:90}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}input,button{font-family:'DM Sans',sans-serif}button{cursor:pointer}
        ...
        .fab{transition:transform 0.15s,box-shadow 0.15s}.fab:hover{transform:scale(1.08);box-shadow:0 6px 28px #7c3aed99!important}
      ...
      <div style={{background:"#0c0e18",borderBottom:"1px solid #141726",padding:"16px 18px 0",position:"sticky",top:0,zIndex:50}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          ...
          <button onClick={()=>{let y=yr,m=mo+1;if(m>11){m=0;y++;}setYr(y);setMo(m);}} style={{background:"#141726",border:"none",color:"#9ca3c0",borderRadius:9,width:32,height:32,fontSize:18,display:"flex",alignItems:"center",justifyContent:"center"}}>›</button>
        </div>
        ...
        <div style={{background:"linear-gradient(135deg,#13103a,#0b1528 50%,#0e1a10)",borderRadius:"14px 14px 0 0",padding:"14px 16px 12px",border:"1px solid #1e2140",borderBottom:"none",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{fontFamily:"'Space Mono',monospace",fontSize:30,fontWeight:700,color:balance>=0?"#a78bfa":"#f87171",letterSpacing:-1,marginBottom:12}}>
        ...
        <div style={{display:"flex",overflowX:"auto",msOverflowStyle:"none",scrollbarWidth:"none"}}>
          {tabs.map(t=>(
            <button key={t.id} onClick={()=>setView(t.id)} style={{flex:"0 0 auto",padding:"10px 12px 8px",background:"none",border:"none",color:view===t.id?"#a78bfa":"#3d4268",fontSize:10,fontWeight:600,letterSpacing:0.5,borderBottom:`2px solid ${view===t.id?"#7c3aed":"transparent"}`,transition:"all 0.15s",whiteSpace:"nowrap"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:11}}>
            ...
            <div className="card" style={{background:"#0c0e18",border:"1px solid #141726",borderRadius:14,padding:16}}>
              ...
              <div style={{height:12,background:"#141726",borderRadius:6,overflow:"hidden",display:"flex",marginBottom:14}}>
                {(income+balCarry)>0&&[{val:totalFixed,color:"#f87171"},...categories.filter(c=>c.spent>0).map(c=>({val:c.spent,color:c.color})),{val:totalSaved,color:"#3b82f6"}].map((seg,i)=>{
                  const w=Math.max((seg.val/(income+balCarry))*100,0);
                  return w>0?<div key={i} className="prog" style={{height:"100%",width:`${Math.min(w,100)}%`,background:seg.color,flexShrink:0}}/>:null;
                })}
              </div>
              {totalFixed>0&&<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div style={{display:"flex",alignItems:"center",gap:7}}><div style={{width:9,height:9,borderRadius:2,background:"#f87171"}}/><span style={{fontSize:12,color:"#9ca3c0"}}>Fixed</span></div><span style={{fontFamily:"'Space Mono',monospace",fontSize:11,color:"#f87171",fontWeight:600}}>-${fmt(totalFixed)}</span></div>}
              {categories.filter(c=>c.budget+c.carryover>0||c.spent>0).map(cat=>{
                const total=cat.budget+cat.carryover,pct=total>0?(cat.spent/total)*100:0,over=cat.spent>total;
                return(
                  <div key={cat.name} style={{marginBottom:8}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                      <div style={{display:"flex",alignItems:"center",gap:7}}><div style={{width:9,height:9,borderRadius:2,background:cat.color}}/><span style={{fontSize:12,color:"#9ca3c0"}}>{cat.emoji} {cat.name}</span></div>
                      <div style={{display:"flex",alignItems:"center",gap:5}}><span style={{fontFamily:"'Space Mono',monospace",fontSize:10,color:over?"#f87171":cat.color,fontWeight:600}}>${fmt(cat.spent)}</span><span style={{fontSize:9,color:"#3d4268"}}>/ ${fmt(total)}</span></div>
                    </div>
                    <div style={{height:3,background:"#141726",borderRadius:2,overflow:"hidden"}}><div className="prog" style={{height:"100%",width:`${Math.min(pct,100)}%`,background:over?"#ef4444":cat.color,borderRadius:2}}/></div>
                  </div>
                );
              })}
              {totalSaved>0&&<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div style={{display:"flex",alignItems:"center",gap:7}}><div style={{width:9,height:9,borderRadius:2,background:"#3b82f6"}}/><span style={{fontSize:12,color:"#9ca3c0"}}>Savings</span></div><span style={{fontFamily:"'Space Mono',monospace",fontSize:11,color:"#3b82f6",fontWeight:600}}>${fmt(totalSaved)}</span></div>}
              <div style={{borderTop:"1px solid #141726",marginTop:4,paddingTop:8,display:"flex",justifyContent:"space-between"}}>
                <span style={{fontSize:12,color:unallocated>=0?"#34d399":"#f87171",fontWeight:600}}>{unallocated>=0?"Free to Allocate":"Over-allocated"}</span>
                <span style={{fontFamily:"'Space Mono',monospace",fontSize:13,fontWeight:700,color:unallocated>=0?"#34d399":"#f87171"}}>{unallocated<0?"-":""}${fmt(Math.abs(unallocated))}</span>
              </div>
            </div>
            ...
            <div className="card" style={{background:"#0c0e18",border:"1px solid #141726",borderRadius:14,padding:16}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                ...
                <button onClick={()=>setView("manage")} style={{fontSize:11,color:"#6b7299",background:"none",border:"none",padding:0}}>Manage accounts →</button>
              </div>
              <div style={{fontFamily:"'Space Mono',monospace",fontSize:22,fontWeight:700,color:"#a78bfa",marginBottom:10}}>${fmt(netWorth)}</div>
              {accounts.length===0&&savings.length===0&&<div style={{fontSize:12,color:"#3d4268",textAlign:"center",padding:"8px 0"}}>Add accounts in Manage to track net worth</div>}
              {accounts.map((a,i)=>(
                <div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:6}}>
                  <span style={{color:"#9ca3c0"}}>{a.emoji} {a.name}</span>
                  <span style={{fontFamily:"'Space Mono',monospace",color:a.balance>=0?"#34d399":"#f87171",fontWeight:600}}>${fmt(a.balance)}</span>
                </div>
              ))}
              {savings.filter(sv=>(sv.totalDeposited||0)+sv.deposited>0).map((sv,i)=>(
                <div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:6}}>
                  <span style={{color:"#9ca3c0"}}>{sv.emoji||"🏦"} {sv.name} (savings)</span>
                  <span style={{fontFamily:"'Space Mono',monospace",color:"#60a5fa",fontWeight:600}}>${fmt((sv.totalDeposited||0)+sv.deposited)}</span>
                </div>
              ))}
            </div>
          </div>
              ...
              return(
                <div key={sv.name} className="card" style={{background:"#0c0e18",border:"1px solid #141726",borderRadius:14,padding:14}}>
                  ...
                  {sv.goal>0&&(
                    <div style={{marginBottom:10}}>
                      <div style={{height:8,background:"#141726",borderRadius:4,overflow:"hidden",marginBottom:4,position:"relative"}}>
                        ...
                        {sv.deposited>0&&<div className="prog" style={{position:"absolute",left:`${Math.min(((sv.totalDeposited||0)/sv.goal)*100,100)}%`,height:"100%",width:`${Math.min((sv.deposited/sv.goal)*100,100)}%`,background:"linear-gradient(90deg,#60a5fa,#a78bfa)",borderRadius:4,boxShadow:"0 0 6px #60a5fa88"}}/>}
                      </div>
                  ...
                  <div style={{display:"flex",gap:7}}>
                    ...
                    <button onClick={()=>setModal({type:"savHistory",sv,svIdx})} style={{padding:"8px 12px",background:"#141726",color:"#6b7299",border:"none",borderRadius:8,fontSize:12}}>History</button>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:10}}>
                  {categories.map(cat=>(
                    <div key={cat.name} style={{display:"flex",alignItems:"center",gap:5,fontSize:10,color:"#6b7299"}}>
                      <div style={{width:10,height:3,background:cat.color,borderRadius:2}}/>
                      {cat.emoji} {cat.name}
                    </div>
                  ))}
                </div>
                ...
                return(
                  <div key={cat.name} style={{marginBottom:10}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        <span style={{fontSize:10,color:"#4b5280"}}>{sharePct.toFixed(0)}% of spend</span>
        ...
        {view==="manage"&&(
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            <div className="card" style={{background:"#0c0e18",border:"1px solid #141726",borderRadius:14,padding:16}}>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}>{PALETTE.map(c=><button key={c} onClick={()=>setCatColor(c)} style={{width:22,height:22,borderRadius:"50%",background:c,border:catColor===c?"3px solid #fff":"3px solid transparent",outline:"none",flexShrink:0,transition:"transform 0.1s"}}/>)}</div>
              <div style={{display:"flex",gap:8}}>
            ...
            <div className="card" style={{background:"#0c0e18",border:"1px solid #141726",borderRadius:14,padding:16}}>
              ...
              <div style={{display:"flex",gap:8,marginBottom:10}}>
                <input value={tmplName} onChange={e=>setTmplName(e.target.value)} placeholder="Template name (e.g. 'My Setup')" style={{flex:1,background:"#141726",border:"1px solid #252840",borderRadius:9,padding:"9px 12px",color:"#e2e4f0",fontSize:13,outline:"none"}}/>
              ...
              {templates.length===0?<div style={{fontSize:12,color:"#3d4268",textAlign:"center",padding:"8px 0"}}>Save your current categories as a reusable template</div>
              :templates.map((t,i)=>(
                <div key={i} style={{background:"#0a0c14",border:"1px solid #1e2140",borderRadius:10,padding:"10px 12px",marginBottom:8}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                    ...
                    <div style={{display:"flex",gap:6}}>
                      <button onClick={()=>setModal({type:"templateLoad",tmpl:t,tmplIdx:i})} style={{padding:"5px 12px",background:"#7c3aed22",color:"#a78bfa",border:"1px solid #7c3aed33",borderRadius:7,fontSize:12,fontWeight:600}}>Load</button>
      ...
      {modal&&(
        <div onClick={()=>setModal(null)} style={{position:"fixed",inset:0,background:"#000000aa",zIndex:100,display:"flex",alignItems:"flex-end"}}>
          <div className="sheet" onClick={e=>e.stopPropagation()} style={{background:"#0c0e18",borderRadius:"20px 20px 0 0",padding:"22px 20px 44px",width:"100%",border:"1px solid #1e2140",borderBottom:"none",maxHeight:"92vh",overflowY:"auto"}}>
            ...
            {modal.type==="templateLoad"&&(
              <>
                ...
                {/* Category preview */}
                <div style={{fontSize:10,color:"#4b5280",letterSpacing:1,textTransform:"uppercase",marginBottom:8}}>Categories in this template</div>
                ...
                {/* Income / fixed summary if present */}
                {(modal.tmpl.income>0||(modal.tmpl.fixedExpenses||[]).length>0)&&(
                  <div style={{background:"#141726",borderRadius:10,padding:"10px 14px",marginBottom:16,display:"flex",gap:16}}>
                    ...
                    {(modal.tmpl.fixedExpenses||[]).length>0&&<div style={{fontSize:12,color:"#f87171"}}>📌 {modal.tmpl.fixedExpenses.length} fixed expense{modal.tmpl.fixedExpenses.length>1?"s":""}</div>}
                  </div>
                )}

                {/* Merge vs Replace explanation */}
                <div style={{background:"#0a0c14",border:"1px solid #1e2140",borderRadius:10,padding:"12px 14px",marginBottom:16,fontSize:12,color:"#6b7299"}}>
                  <div style={{marginBottom:6}}><span style={{color:"#a78bfa",fontWeight:700}}>Replace</span> — clears your current categories and loads this template fresh. Income {"&"} fixed expenses also replaced if saved.</div>
                  <div><span style={{color:"#34d399",fontWeight:700}}>Merge</span> — adds template categories that don't already exist. Your current setup stays intact.</div>
                </div>

                <div style={{display:"flex",gap:8,marginBottom:8}}>
                  <button onClick={()=>{
                    const newCats=modal.tmpl.categories.map(c=>mkCat(c.name,c.emoji,c.color,c.budget));
                    const newInc=modal.tmpl.income||income;
                    const newFix=(modal.tmpl.fixedExpenses||[]).length>0?modal.tmpl.fixedExpenses:fixed;
                    persist(newCats,newInc,null,newFix);
                    setModal(null);showToast("Template loaded ✓");
                  }} style={{flex:1,padding:"13px 0",background:"linear-gradient(135deg,#7c3aed,#5b21b6)",color:"#fff",border:"none",borderRadius:11,fontWeight:700,fontSize:14}}>Replace</button>
                  <button onClick={()=>{
                    const existing=new Set(categories.map(c=>c.name.toLowerCase()));
                    const toAdd=modal.tmpl.categories.filter(c=>!existing.has(c.name.toLowerCase())).map(c=>mkCat(c.name,c.emoji,c.color,c.budget));
                    if(toAdd.length===0){showToast("All categories already exist","err");setModal(null);return;}
                    persist([...categories,...toAdd],null,null,null);
                    setModal(null);showToast(`${toAdd.length} categor${toAdd.length===1?"y":"ies"} merged ✓`);
